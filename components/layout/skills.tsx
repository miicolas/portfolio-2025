...
export const Skill = ({ className, logo, name, style }: { className: string, logo: string, name: string, style: React.CSSProperties }) => {
    return (
        <Card className={`p-5 rounded-lg bg-neutral-100 dark:bg-neutral-900 absolute w-fit h-fit ${className}`} style={style}>
            <Image src={logo} alt={name} width={1200} height={1200} className="w-12 h-12 m-auto" />
        </Card>
    )
}
